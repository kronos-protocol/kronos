...
static Frame s_create_frame(const uint8_t* buffer, const uint16_t received_bytes, uint8_t* frame_data_out, uint16_t stack_data_out_size) {
    uint16_t data_length = _krs_frame_calculate_body_length(received_bytes);
    ...
    Frame frame;
    frame.received_bytes = received_bytes;
    ...
    memcpy(frame_data_out, buffer + KRONOS_FRAME_HEADER_LENGTH, _krs_frame_calculate_body_length(received_bytes));
    //TODO: initialize channel and packet_id
    return frame;
}
...
Frame* krs_frame_create_heap(const uint8_t* buffer, const uint16_t received_bytes) {
    ...
    uint16_t frame_data_size = _krs_frame_calculate_body_length(received_bytes);
